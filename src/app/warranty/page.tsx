  ...
  return (
    <div className="max-w-3xl mx-auto py-8 sm:py-12 lg:py-16 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-8 sm:mb-10">
        <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 mb-3 sm:mb-4">
          Warranty Tracker
        </h1>
        <p className="text-base sm:text-lg text-slate-500 max-w-xl mx-auto font-medium">
          Enter your product's Unique ID or scan the QR code to check your
          warranty status, validity, and claim options.
        </p>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-4 sm:mb-8">
            ...
            <div className="flex space-x-3 sm:space-x-4">
              <Button
                ...
                disabled={isFetching}
                className="flex-1 sm:flex-none h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base rounded-xl sm:rounded-2xl shadow-lg shadow-blue-500/25"
              >
                {isFetching ? "Checking..." : "Check Status"}
              </Button>
              <Button
                variant="outline"
                className="h-12 w-12 sm:h-14 sm:w-14 p-0 shrink-0 rounded-xl sm:rounded-2xl border-slate-200"
                title="Scan QR Code"
              >
            ...
            <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 bg-slate-50/80 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-slate-100/50">
                <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-100">
                  ...
                  <p className="font-mono text-sm sm:text-base font-bold text-slate-800">
                    {data.uniqueId}
                  </p>
                </div>
                <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-100">
              ...
              {data.status === "ACTIVE" && (
                <div className="mt-6 flex flex-col sm:flex-row sm:items-center justify-between bg-blue-50/80 border border-blue-100/50 p-4 sm:p-5 rounded-xl sm:rounded-2xl gap-4 sm:gap-0">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-blue-700/80 font-medium mt-0.5">
                      You are eligible for warranty claims.
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    className="w-full sm:w-auto bg-white text-blue-700 border-blue-200 hover:bg-blue-50 rounded-xl shadow-sm h-10 sm:h-12"
                  >
                    Request Claim
                  </Button>
                </div>
