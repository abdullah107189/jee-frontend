...
export default function HomePage() {
  ...
  const handleQuickCheck = (e: React.FormEvent) => {
    ...
    if (quickId.trim()) {
      router.push(
        `/warranty?id=${encodeURIComponent(quickId.trim().toUpperCase())}`,
      );
    }
  };
  ...
  const categories = [
    ...
    const apiCategory = categoriesData.find(
      (entry) => entry.name.toLowerCase() === category.name.toLowerCase(),
    );
    return {
      ...category,
      count: apiCategory?.productCount
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              <div className="lg:col-span-7 space-y-6">
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-400/20 text-blue-400 text-xs font-extrabold uppercase tracking-wider">
                  <ShieldCheck className="h-4 w-4" /> Official Brand Guarantee
                  System
                </div>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight leading-tight">
                  Authentic Appliances with{" "}
                ...
                <p className="text-base sm:text-lg text-slate-300 font-medium max-w-2xl leading-relaxed">
                  Shop genuine electronics online or verify offline seller
                  purchases instantly via Unique Product IDs & QR codes. Enjoy
                  70 BDT bKash advance checkout!
                </p>
                <div className="flex flex-wrap gap-4 pt-2">
                  <Link href="/products">
                    <Button
                      size="lg"
                      className="h-13 px-8 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-bold shadow-xl shadow-blue-600/30 text-base"
                    >
                  ...
                  <Link href="/warranty">
                    <Button
                      size="lg"
                      variant="outline"
                      className="h-13 px-8 rounded-full border-slate-700 bg-slate-800/80 hover:bg-slate-800 text-white font-bold text-base"
                    >
              ...
              <div className="lg:col-span-5">
                <Card className="rounded-3xl border-slate-800 bg-slate-900/90 backdrop-blur-xl p-6 sm:p-8 shadow-2xl text-white border">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-medium">
                        Verify your offline or online product status
                      </p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleQuickCheck} className="space-y-4">
                    ...
                    <Button
                      type="submit"
                      className="w-full h-12 rounded-2xl bg-emerald-600 hover:bg-emerald-500 font-bold text-white shadow-lg shadow-emerald-600/30"
                    >
              ...
              return (
                <Link key={cat.name} href={`/products?category=${cat.name}`}>
                  <Card className="rounded-3xl border-slate-200/80 hover:border-blue-500 hover:shadow-xl transition-all duration-300 group bg-white overflow-hidden text-center p-6">
                    <div
                      className={`h-16 w-16 mx-auto rounded-2xl ${cat.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                    >
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {latestProducts.slice(0, 3).map((product) => (
              <Card
                key={product.id}
                className="rounded-3xl border-slate-200/80 bg-white p-4 flex gap-4 hover:shadow-xl transition-all"
              >
                <img
                  src={product.image}
                  alt={product.name}
...
// Home er section gula alad alada kore vag kore nite hobe !
