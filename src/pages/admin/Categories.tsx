...
const MOCK_CATEGORIES = [
  { id: '1', name: 'Electronics', slug: 'electronics', level: 'Parent', status: 'Active' },
  { id: '2', name: 'Mobile Phones', slug: 'mobile-phones', level: 'Child (Electronics)', status: 'Active' },
  { id: '3', name: 'Furniture', slug: 'furniture', level: 'Parent', status: 'Active' },
];
    ...
    <DashboardLayout allowedRole="admin">
      <div className="space-y-6 sm:space-y-8">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 sm:gap-0">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900">Categories</h1>
            <p className="text-sm sm:text-base text-slate-500 mt-1 font-medium">Manage product categories and taxonomy.</p>
          </div>
          <Button className="h-10 sm:h-12 rounded-xl sm:rounded-2xl shadow-lg shadow-blue-500/20">
            <Plus className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Add Category
          </Button>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50/80 border-b border-slate-100">
                <tr>
                  <th className="px-4 sm:px-6 py-4 sm:py-5 font-medium text-slate-500 whitespace-nowrap">Name</th>
                  <th className="px-4 sm:px-6 py-4 sm:py-5 font-medium text-slate-500 whitespace-nowrap">Slug</th>
                  <th className="px-4 sm:px-6 py-4 sm:py-5 font-medium text-slate-500 whitespace-nowrap">Level</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {MOCK_CATEGORIES.map((category) => (
                  <tr key={category.id} className="hover:bg-slate-50/50 transition-colors bg-white">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <Badge variant={category.status === 'Active' ? 'success' : 'outline'} className="rounded-full px-3 py-1 text-xs">
                        {category.status}
                      </Badge>
                    </td>
